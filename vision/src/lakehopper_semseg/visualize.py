...
def imshow_rgb(plt, image: np.ndarray):
    if isinstance(image, tf.Tensor):
        image = image.numpy()
    image = image / 255
    plt.imshow(image, interpolation="none", aspect="equal", vmin=0, vmax=1)


def imshow_mask(plt, mask: np.ndarray):
    if isinstance(mask, tf.Tensor):
        mask = mask.numpy()
    # Convert from BGR to RGB
    # https://stackoverflow.com/a/50630968/7120579
    mask = mask[..., ::-1]
    plt.imshow(mask, interpolation="none", aspect="equal", vmin=0, vmax=1)
...
def show(rows: Union[list[ImgMask], ImgMask]):
    ...
    plt.figure(figsize=(11, nbro_rows * 4), dpi=100)
    for y, row in enumerate(rows):
        for x, (image, title) in enumerate(zip(row, IMAGE_TITLES)):
            plt.subplot(nbro_rows, nbro_columns, y * nbro_columns + x + 1)
            plt.title(f"{title} {y}")
            if x == 0:
                imshow_rgb(plt, image)
            elif image.shape[-1] == 3:
                imshow_mask(plt, image)
            plt.axis("off")
    plt.show()
