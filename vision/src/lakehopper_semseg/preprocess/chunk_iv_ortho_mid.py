...
import shapely.ops
import numpy as np
from preprocess.colormaps import IV_ORTHO_MID_LABEL_COLOR_MAP
...
def pixel_to_geo_coord(geo_transform, pixel_coord):
    ...
    return(
        a + b * pixel_x + c * pixel_y,
        d + e * pixel_x + f * pixel_y
    )
...
def jp2_to_gtif(jp2: glymur.Jp2k):
    gtif_data = next(box for box in jp2.box if isinstance(box, jp2box.UUIDBox) and box.uuid == jp2box._GEOTIFF_UUID)
...
def chunk_iv_ortho_mid(base_path: Path, map_sheet_code: Path):
    ...
    missing_codes = set(CATEGORY_CODE_LABEL_MAP.keys()).difference(category_zip_paths.keys())
    ...
    for category_code, grb_zip_path in category_zip_paths.items():
        vfs_path = f'zip://{grb_zip_path}!/Shapefile/{category_code}{map_sheet_code}.shp'
    ...
    for chunk_y, chunk_x in itertools.product(range(nbro_chunks_y), range(nbro_chunks_x)):
        ...
        # start bottom left
        (geo_x_min, geo_y_max) = pixel_to_geo_coord(geo_transform, (ortho_x_min, ortho_y_min))
        (geo_x_max, geo_y_min) = pixel_to_geo_coord(geo_transform, (ortho_x_max, ortho_y_max))
        ...
        ground_color = IV_ORTHO_MID_LABEL_COLOR_MAP['ground']
        chunk_mask = np.tile(np.array(ground_color, dtype='uint8'), (chunk_ortho_h, chunk_ortho_w, 1))
        ...
        for label, polygons in label_polygons_map.items():
            ...
            if isinstance(clipped, Polygon):
                ...
                raise RuntimeError(f'Unexpected clipped geometry type: {clipped.__class__}')
        ...
        cv2.imwrite(str(mask_chunks_path / name), cv2.cvtColor(chunk_mask, cv2.COLOR_RGB2BGR))
...
if __name__ == '__main__':
    ...
    parser.add_argument('BASE_PATH', type=Path, help='Path to directory containing jp2 file and GBG zips')
