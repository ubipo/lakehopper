...
from .colormaps import DRONEDEPLOY_IGNORE_COLOR, DRONEDEPLOY_LABEL_COLOR_MAP, IV_ORTHO_MID_LABEL_COLOR_MAP
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', required=True, choices=[DATASET_DRONEDEPLOY, DATASET_IV_ORTHO_MID], type=str, help='Dataset to chippify')
    ...
    labels_to_keep = ['building', 'water', 'ground']
    replacement_label = 'ground'
    ...
    if dataset == DATASET_DRONEDEPLOY:
        ...
        image_path_to_prefix = lambda image_path: chips_prefix + image_path.stem.removesuffix('-ortho')
        ...
        image_path_to_prefix = lambda image_path: image_path.stem
    else:
        raise RuntimeError(f'Unknown dataset: {dataset}')

    images_to_chips(
        images_paths, labels_paths, out_dir, image_path_to_prefix,
        labels_to_keep, replacement_label,
        label_color_map, ignore_color
    )
