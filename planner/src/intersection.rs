//! Adaptation of Ulysse Carion line_intersection crate.
//! See: https://github.com/ucarion/line_intersection
//! Patched with: https://github.com/ucarion/line_intersection/pull/3
//!
...
use geo::{prelude::ClosestPoint, Coordinate, Line, Point};
...
pub struct LineInterval<T: Float + std::fmt::Debug> {
    pub line: Line<T>,
...
impl<T: Float + std::fmt::Debug> LineInterval<T> {
    ...
    fn t_coord_to_point(p: &Point<T>, r: &Point<T>, t: T) -> Coordinate<T> {
        (p.x() + t * r.x(), p.y() + t * r.y()).into()
    }
}

/// Calculates the proper intersection between `ray` and `line`.
/// 'Proper' means any intersection that is not the start or end of `line`.
/// This was not part of ucarion/line_intersection.
pub fn get_proper_ray_line_intersection(
    ray: Line<f64>,
    line: &Line<f64>,
) -> std::option::Option<Coordinate<f64>> {
    let intersection = match LineInterval::ray(ray).relate(&LineInterval::line_segment(*line)) {
        crate::intersection::LineRelation::DivergentIntersecting(c) => Some(c),
        crate::intersection::LineRelation::DivergentDisjoint => None,
        crate::intersection::LineRelation::Collinear => {
            match line.closest_point(&ray.start.into()) {
                geo::Closest::Intersection(p) => Some(p.0),
                geo::Closest::SinglePoint(p) => Some(p.0),
                geo::Closest::Indeterminate => None,
            }
        }
        crate::intersection::LineRelation::Parallel => None,
    }?;
    if intersection == line.start || intersection == line.end {
        return None;
    }
